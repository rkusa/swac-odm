      ...
      set: function(newValue) {
        ...
        if (Array.isArray(newValue)) {
          ...
          if (scheme.type === 'array') {
            ...
            values[key] = Observable.Array.isObservableArray(newValue)
              ? newValue.clone()
              : new Observable.Array(newValue)
  ...
  this._definition.properties.forEach(function(property) {
    ...
    if (scheme.type === 'array') {
      ...
      self[property] = scheme.embedded
        ? new Observable.Array(values, scheme.embedded)
        : new Observable.Array(values)
      if (!scheme.silent) {
        self[property].on('changed', proxy(self, 'emit', self, 'changed'))
        self[property].on('changed', proxy(self, 'emit', self, 'changed.' + property))
        self[property].on('changed', proxy(self, 'triggerChanged'))
      }
