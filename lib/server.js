    ...
    .chain(function(done) {
      ...
      if (method === 'view') {
        ...
        if (viewId in viewCache) {
          ...
          return callback(null, [].concat(viewCache[viewId]))
        }
        ...
        Model.api[model._type].view.apply(
          ...
          args.concat(function(err, rows) {
            if (err) throw err
            viewCache[viewId] = rows.map(function(row) {
              var clone = row.clone()
              cache.docs[row.id] = clone
              return clone
            })
            done(input, rows)
          })
        ...
        if (id in cache.docs) {
          return done(input, cache.docs[id] ? cache.docs[id].clone() : null)
        }

        Model.api[model._type].get(id, function(err, row) {
          if (err) throw err
          cache.docs[id] = row ? row.clone() : null
          done(input, row)
        })
      }
    })
      ...
      authorize(model, method, allow, deny,
        ...
        function() {
          ...
          .chain(function(done) {
            ...
            // Execute Write
            .chain(function(done) {
              Model.api[model._type][method](input, function(err) {
                if (err) throw err
