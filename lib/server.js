...
exports.getMethodFn = function(operation, model, isAPICall) {
  ...
  return function() {
    ...
    var viewId, viewCache
    if (method === 'view') {
      var query = typeof args[args.length - 1] === 'object' ? args.pop() : {}
      viewId = ((args.join('/') || 'all') + '?' + Object.keys(query).map(function(key) {
    ...
    .chain(function(done) {
      ...
      if (method === 'view') {
        ...
        model._definition.api.view.apply(
          ...
          args.concat(query, function(err, rows) {
            if (err) throw err
            done(input, rows)
          })
        )
    ...
    .chain(function (input, instance, done) {
      ...
      if (method === 'put' && !instance) method = 'post'
        ...
        return callback(new APIError(404, 'Not Found'))
      }
      ...
      authorize(model, method, allow, deny,
          ...
          .chain(function(done) {
            ...
            .chain(function(done) {
              ...
              executeHook(model, input, 'beforeSave', function(err) {
                if (err) return callback(new APIError(400, 'Bad Request', {
                  error: 'Save Rejected',
                  details: err
                }))
            ...
            .chain(function(done) {
              ...
              input.validate(function(isValid) {
                if (!isValid) {
                  if (callback) {
                    callback(new APIError(400, 'Bad Request', {
        ...
        // denied
        function() {
          if (callback) callback(new APIError(403, 'Forbidden'))
        },
        isAPICall
    ...
    .chain(function(rows) {
      ...
      utils.series(input, function(row, next) {
        ...
        // Cache Result
        var result = Array.isArray(rows) ? output : (output[0] || null)
        if (method === 'view') {
          viewCache[viewId] = result
          result = Array.isArray(result)
            ? result.map(cacheRow)
            : cacheRow(result)
        } else {
          result = cacheRow(result)
        }
